...
SUBMIT_BIN="${RUN_DIR}/scripts/agent"
AGENT_ZIP="${RUN_DIR}/assets/agent-app/agent-app.zip"
LOGROTATE_SRC="${RUN_DIR}/config/logrotate/agent-app"
SYSTEMD_UNIT_SRC="${RUN_DIR}/config/systemd/agent-app.service"
...
printf '적용 범위: SSH 20022/root 차단, 방화벽, agent 계정/권한, 앱/monitor/report 배치, cron 등록, logrotate 설정, 15034 포트 정리\n'
printf '백업: SSH 설정과 crontab/방화벽 상태는 timestamp 파일로 보존합니다.\n'

log_block_start "Install packages"
printf '[실행] 패키지 설치\n'
if [ "$(id -u)" -eq 0 ] && command -v apt-get >/dev/null 2>&1; then
  run_cmd "필수 패키지 설치" "DEBIAN_FRONTEND=noninteractive apt-get update || true; DEBIAN_FRONTEND=noninteractive apt-get install -y openssh-server ufw cron unzip python3 psmisc procps logrotate"
else
...
log_block_start "Readiness checks"
printf '[실행] 설치 전 점검\n'
readiness_ok=0
...
[ -s "${SUBMIT_BIN}/monitor.sh" ] || readiness_ok=1
[ -s "${SUBMIT_BIN}/report.sh" ] || readiness_ok=1
[ -s "$LOGROTATE_SRC" ] || readiness_ok=1
[ -s "$SYSTEMD_UNIT_SRC" ] || readiness_ok=1
if [ "$readiness_ok" -ne 0 ]; then
  {
    printf '[cmd] readiness checks\n'
    printf '[stdout/stderr]\n'
    printf 'root_uid=%s agent_zip=%s monitor=%s report=%s logrotate_src=%s systemd_unit=%s\n' "$(id -u)" "$AGENT_ZIP" "${SUBMIT_BIN}/monitor.sh" "${SUBMIT_BIN}/report.sh" "$LOGROTATE_SRC" "$SYSTEMD_UNIT_SRC"
    printf '[exit] 2\n'
    printf '[interpretation] BLOCKED: root 권한 또는 필수 명령/소스 파일이 부족해 설치/수리를 중단한다.\n\n'
  } >> "$WORK_LOG"
  printf '[실패] 설치 전 점검: root 권한 또는 필수 파일/명령 부족\n최종 종료 코드: 2\n전체 로그: runtime/work.log\n'
  exit 2
fi
{
  printf '[cmd] readiness checks\n'
  printf '[stdout/stderr]\n'
  printf 'root_uid=0 agent_zip=%s monitor=%s report=%s logrotate_src=%s systemd_unit=%s\n' "$AGENT_ZIP" "${SUBMIT_BIN}/monitor.sh" "${SUBMIT_BIN}/report.sh" "$LOGROTATE_SRC" "$SYSTEMD_UNIT_SRC"
  printf '[exit] 0\n'
...
# 재실행 가능성: 기존 service ops block을 교체해 같은 설정을 유지한다.
# 백업/복구: 변경 전 sshd_config를 timestamp 백업으로 보존한다.
log_block_start "Configure OpenSSH"
printf '[실행] SSH 설정\n'
if [ -f /etc/ssh/sshd_config ]; then
  backup="/etc/ssh/sshd_config.agent-service-ops.$(date +%Y%m%dT%H%M%S%z).bak"
  run_cmd "SSH 설정 백업" "cp /etc/ssh/sshd_config '$backup'"
  run_cmd "SSH Port/PermitRootLogin 설정 적용" "sed -i '/^# BEGIN agent service ops$/,/^# END agent service ops$/d' /etc/ssh/sshd_config && sed -i -E 's/^([[:space:]]*Port[[:space:]]+)/# agent service ops disabled: \\1/' /etc/ssh/sshd_config && sed -i -E 's/^([[:space:]]*PermitRootLogin[[:space:]]+)/# agent service ops disabled: \\1/' /etc/ssh/sshd_config && printf '\\n# BEGIN agent service ops\\nPort ${SSH_PORT}\\nPermitRootLogin no\\n# END agent service ops\\n' >> /etc/ssh/sshd_config"
  if command -v sshd >/dev/null 2>&1 || [ -x /usr/sbin/sshd ]; then
    sshd_bin="$(command -v sshd || printf /usr/sbin/sshd)"
    run_cmd "sshd 런타임/host key 준비" "mkdir -p /run/sshd && ssh-keygen -A"
    ...
    if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
      ...
      run_cmd "ssh 서비스 reload/start" "service ssh restart || service sshd restart || service ssh reload || service sshd reload || '$sshd_bin'"
    else
      run_cmd "sshd 직접 start" "'$sshd_bin'"
    fi
  else
...
log_block_start "Configure firewall"
printf '[실행] 방화벽 설정\n'
...
log_block_start "Configure users and groups"
printf '[실행] 계정/그룹 설정\n'
...
# 무엇: AGENT_HOME, 로그/업로드/키 디렉토리와 키 파일을 만든다.
...
log_block_start "Install app and scripts"
printf '[실행] 앱/스크립트 배치\n'
arch="$(uname -m)"
case "$arch" in
...
run_cmd "제공 앱 바이너리 추출" "python3 -c 'import os,sys,zipfile; z,m,t=sys.argv[1:4]; data=zipfile.ZipFile(z).read(m); os.makedirs(os.path.dirname(t), exist_ok=True); open(t, \"wb\").write(data); os.chmod(t, 0o750)' '$AGENT_ZIP' '$app_member' '${AGENT_HOME}/bin/$app_member'"
...
run_cmd "구형 log retention 스크립트 제거" "rm -f '${AGENT_HOME}/bin/log_retention.sh'"

log_block_start "Install systemd service"
printf '[실행] systemd 서비스 등록\n'
run_cmd "agent systemd unit 설치" "install -D -o root -g root -m 644 '$SYSTEMD_UNIT_SRC' /etc/systemd/system/agent-app.service"
if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
  run_cmd "agent systemd daemon-reload/enable" "systemctl daemon-reload && systemctl enable agent-app.service"
else
  run_cmd "systemd 사용 가능 여부" "printf '%s\n' 'systemd가 활성화되지 않아 unit enable은 건너뜁니다.'"
fi

# 무엇: logrotate에 agent 로그 회전/압축 정책을 등록한다.
# 예상여파: /etc/logrotate.d/agent-app 파일이 생성 또는 갱신된다.
# 효과: monitor.log, boot log, cron output이 크기 기준으로 회전되고 압축된다.
# 실패 시 처리: logrotate dry-run 실패를 기록한다.
# 재실행 가능성: install로 같은 정책 파일을 덮어써 같은 상태를 유지한다.
# 백업/복구: 기존 정책 파일은 timestamp 백업으로 보존한다.
log_block_start "Configure logrotate"
printf '[실행] logrotate 설정\n'
if [ -f /etc/logrotate.d/agent-app ]; then
  run_cmd "기존 logrotate 정책 백업" "cp /etc/logrotate.d/agent-app '${RUNTIME_DIR}/archive/agent-app.logrotate.$(date +%Y%m%dT%H%M%S%z).bak'"
fi
run_cmd "agent logrotate 정책 설치" "install -D -o root -g root -m 644 '$LOGROTATE_SRC' /etc/logrotate.d/agent-app"
run_cmd "agent logrotate dry-run" "logrotate -d /etc/logrotate.d/agent-app"
...
log_block_start "Configure cron"
printf '[실행] cron 등록\n'
...
log_block_start "Free agent port"
